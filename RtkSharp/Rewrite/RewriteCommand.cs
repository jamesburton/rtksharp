...
/// <c>evaluate</c> (<c>src/hooks/rewrite_cmd.rs:48</c>), adapted for RtkSharp's no-user-config
/// scope: <see cref="Rewrite.Permissions.CheckCommand"/> and <see cref="Rewrite.RewriteEngine.RewriteCommand"/>
/// are always invoked with empty rule/exclude/prefix lists, since <c>~/.claude/settings.json</c>
/// loading is out of scope for this port.
/// </summary>
public static class RewriteCommand
{
    ...
    /// <see cref="PermissionVerdict.Allow"/> (requires config-sourced allow rules; unreachable
    /// with RtkSharp's current no-config permission evaluation).</description></item>
    /// <item><description><c>3</c>, rewritten output — a rewrite was found and the verdict is
    /// <see cref="PermissionVerdict.Ask"/> (the default outcome with no user config).</description></item>
    /// </list>
    /// </remarks>
    ...
    {
        var verdict = Permissions.CheckCommand(cmd);
