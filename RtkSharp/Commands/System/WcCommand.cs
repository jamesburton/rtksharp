...
public static class WcCommand
{
    ...
    internal static WcMode DetectMode(string[] args)
    {
    ...
    internal static string FilterWcOutput(string raw, WcMode mode)
    {
        var lines = ReadCommand.SplitLines(raw.Trim());
    ...
    internal static string FindCommonPrefix(IReadOnlyList<string> paths)
    {
        if (paths.Count <= 1)
        {
            return string.Empty;
    ...
    internal static string StripPrefix(string path, string prefix)
    {
        if (prefix.Length == 0)
        {
            return path;
