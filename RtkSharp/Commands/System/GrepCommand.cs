...
public static class GrepCommand
{
    ...
    /// <summary>
    /// Config-aware wrapper around <see cref="GrepFilters.BuildGroupedOutput"/>: resolves the
    /// user's configured <c>limits.grep_max_per_file</c> (via <see cref="GrepMaxPerFile"/>) and
    /// passes it through, so callers (and the config-wiring test suite) that need the real
    /// configured cap — rather than <see cref="GrepFilters"/>'s dependency-free default — don't
    /// have to duplicate the config lookup themselves.
    /// </summary>
    ...
    internal static string BuildGroupedOutput(
        ...
        GrepFilters.BuildGroupedOutput(rawOutput, patternDisplay, maxLen, maxResults, contextOnly, GrepMaxPerFile());
    ...
    {
        ArgumentNullException.ThrowIfNull(stdout);
        return RtkSharp.Core.SourceFilterLineSplitter.SplitLines(stdout).Count(line =>
        {
            var trimmed = line.Trim();
            return trimmed.Length != 0 && trimmed != "--" && GrepFilters.ParseMatchLine(line) is null;
        });
    }
