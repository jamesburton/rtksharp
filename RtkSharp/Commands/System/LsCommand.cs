...
/// line is a stderr diagnostic that does not affect filtered output.
/// </remarks>
public static partial class LsCommand
{
    /// <summary>
    ...
    public static Task<int> RunAsync(string[] args)
    {
        ...
        var showAll = DetectShowAll(args);
        var showLong = DetectShowLong(args);
        ...
        return CommandRunner.RunFilteredAsync(
            ...
            "ls",
            $"-la {targetDisplay}",
            raw => ApplyFilter(raw, showAll, showLong, isTty),
    ...
    /// <summary>
    /// Core filter shared by <see cref="RunAsync"/> and <see cref="FilterLs"/>: runs
    ...
    private static string ApplyFilter(string raw, bool showAll, bool showLong, bool includeSummary)
    {
    ...
    internal static bool DetectShowAll(string[] args) =>
    ...
    internal static bool DetectShowLong(string[] args) =>
        args.Any(a =>
        {
    ...
    internal static string HumanSize(ulong bytes)
    {
        if (bytes >= 1_048_576)
        {
    ...
    internal static (char FileType, string Perms, ulong Size, string Name)? ParseLsLine(string line)
    {
    ...
    internal static string? PermsToOctal(string perms)
    {
    ...
    /// </returns>
    internal static (string Entries, string Summary, int ParsedCount) CompactLs(
