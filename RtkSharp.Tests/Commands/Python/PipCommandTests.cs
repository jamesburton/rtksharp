...
/// <summary>
/// Dispatch-level coverage for the <c>rtk pip</c> entry point — unlike
/// <see cref="RuffCommand"/>/<see cref="PytestCommand"/>/<see cref="MypyCommand"/> (all
/// non-injectable, buffered-through-<c>CommandRunner</c> commands), <see cref="PipCommand"/> takes an
/// injectable <see cref="IProcessExecutor"/> and tool-existence predicate — the same shape
/// <c>DockerCommandTests</c> established for <see cref="Cloud.DockerCommand"/> — so its subcommand
/// dispatch (list/outdated/passthrough) and the <c>pip</c>-missing/<c>uv</c>-fallback branch can be
/// exercised deterministically. The pure filter-function tests (<c>filter_pip_list</c>/
/// <c>filter_pip_outdated</c>) moved to
/// <c>RtkSharp.Filters.Tests.Commands.Python.PipFiltersTests</c> when the underlying methods moved
/// from <c>RtkSharp.Commands.Python.PipFilters</c> to
/// <see cref="RtkSharp.Filters.Commands.Python.PipFilters"/> (Task 10 of the filters-library
/// extraction).
/// </summary>
