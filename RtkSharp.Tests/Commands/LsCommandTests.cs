...
/// <summary>
/// Tests for <see cref="LsCommand"/>. The filter is a pure function, so the fixture-based
...
/// </summary>
public sealed class LsCommandTests
{
    private static string FixturesDir([CallerFilePath] string? thisFile = null) =>
        Path.Combine(Path.GetDirectoryName(thisFile)!, "..", "Fixtures", "ls");
    ...
    public void FilterLs_PlainDirectory_MatchesOracle()
    {
        ...
        var actual = LsCommand.FilterLs(raw, showAll: false, showLong: false);
    ...
    public void FilterLs_LongMode_ShowsOctalPermsMatchingOracle()
    {
        ...
        var actual = LsCommand.FilterLs(raw, showAll: false, showLong: true);
    ...
    public void FilterLs_HiddenFilesWithAll_KeepsNoiseDirs()
    {
        ...
        var actual = LsCommand.FilterLs(raw, showAll: true, showLong: false);
    ...
    public void DetectShowAll_MatchesRustSemantics(string[] args, bool expected) =>
        Assert.Equal(expected, LsCommand.DetectShowAll(args));
    ...
    public void DetectShowLong_MatchesRustSemantics(string[] args, bool expected) =>
        Assert.Equal(expected, LsCommand.DetectShowLong(args));
    ...
    public void HumanSize_MatchesRust(ulong bytes, string expected) =>
        Assert.Equal(expected, LsCommand.HumanSize(bytes));
    ...
    public void PermsToOctal_Common(string perms, string expected) =>
        Assert.Equal(expected, LsCommand.PermsToOctal(perms));
    ...
    public void PermsToOctal_Garbage_ReturnsNull(string perms) =>
        Assert.Null(LsCommand.PermsToOctal(perms));
    ...
    {
        var result = LsCommand.ParseLsLine("-rw-r--r--  1 user staff 1234 Jan  1 12:00 file.txt");
    ...
    {
        var result = LsCommand.ParseLsLine(
    ...
    {
        var result = LsCommand.ParseLsLine("lrwxr-xr-x  1 user staff 10 Jan  1 12:00 link -> target");
        Assert.NotNull(result);
    ...
    {
        var result = LsCommand.ParseLsLine("-rw-r--r--  1 user staff 5678 Dec 25  2024 old.tar.gz");
        Assert.NotNull(result);
    ...
    public void ParseLsLine_TotalHeader_ReturnsNull() =>
        Assert.Null(LsCommand.ParseLsLine("total 48"));
    ...
    public void CompactLs_Basic_GroupsDirsAndRendersSizes()
    {
        ...
        var (entries, _, _) = LsCommand.CompactLs(input, showAll: false, showLong: false);
    ...
    public void CompactLs_ShowAll_KeepsNoiseDirs()
    {
        const string input =
            ...
            "drwxr-xr-x  2 user  staff  64 Jan  1 12:00 .git\n" +
            "drwxr-xr-x  2 user  staff  64 Jan  1 12:00 src\n";
        var (entries, _, _) = LsCommand.CompactLs(input, showAll: true, showLong: false);
    ...
    {
        var (entries, summary, _) = LsCommand.CompactLs("total 0\n", showAll: false, showLong: false);
        Assert.Equal("(empty)\n", entries);
    ...
    public void CompactLs_OnlyDotDirs_ReportsEmpty()
    {
        ...
        var (entries, summary, parsedCount) = LsCommand.CompactLs(input, showAll: false, showLong: false);
    ...
    public void CompactLs_Summary_CountsFilesDirsAndExtensions()
    {
        ...
        var (_, summary, _) = LsCommand.CompactLs(input, showAll: false, showLong: false);
    ...
    public void CompactLs_LongFormat_IncludesOctal()
    {
        ...
        var (entries, _, _) = LsCommand.CompactLs(input, showAll: false, showLong: true);
    ...
    public void FilterLs_EntriesNeverContainSummary()
    {
        const string input =
            ...
            "drwxr-xr-x  2 user  staff    64 Jan  1 12:00 src\n" +
            "-rw-r--r--  1 user  staff  1234 Jan  1 12:00 main.rs\n";
        var filtered = LsCommand.FilterLs(input, showAll: false, showLong: false);
