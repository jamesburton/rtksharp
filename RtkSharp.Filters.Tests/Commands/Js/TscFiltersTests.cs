...
/// <summary>
/// Tests for <see cref="TscFilters"/>, moved from <c>RtkSharp.Tests.Commands.Js.TscCommandTests</c>
/// (Task 7 of the filters-library extraction) — ported directly from Rust's own <c>#[cfg(test)]</c>
/// module in <c>tsc_cmd.rs</c> — specifically the buffered <c>filter_tsc_output</c> tests
...
/// <see cref="TscFilters.TscErrorRegex"/> pattern itself.
/// </summary>
public sealed class TscFiltersTests
{
    // -----------------------------------------------------------------------
    ...
    public void FilterTscOutput_GroupsByFileAndReplacesSummaryLine()
    {
        ...
        var result = TscFilters.FilterTscOutput(output);
    ...
    {
        var result = TscFilters.FilterTscOutput("Found 0 errors. Watching for file changes.");
    ...
    public void FilterTscOutput_NoMatchesAndNoFoundZeroErrorsSubstring_ReturnsNoErrorsFoundMessage()
    {
        ...
        var result = TscFilters.FilterTscOutput("Compilation finished with 0 issues.");
        Assert.Equal("TypeScript: No errors found", result);
    }
    ...
    public void FilterTscOutput_EmptyOutput_ReturnsNoErrorsFoundMessage()
    {
        ...
        var result = TscFilters.FilterTscOutput(string.Empty);
        Assert.Equal("TypeScript: No errors found", result);
    }
    ...
    public void FilterTscOutput_MoreThanFiveDistinctCodes_ShowsOnlyTop5ByFrequency()
    {
        ...
        var result = TscFilters.FilterTscOutput(lines.ToString());
    ...
    public void TscErrorRegex_MatchesRepresentativeDiagnosticLines(string line) =>
        Assert.Matches(TscFilters.TscErrorRegex(), line);
    ...
    public void TscErrorRegex_DoesNotMatchNonDiagnosticLines(string line) =>
        Assert.DoesNotMatch(TscFilters.TscErrorRegex(), line);

    [Fact]
    public void TscErrorRegex_CapturesGroupsCorrectly()
    {
        var match = TscFilters.TscErrorRegex().Match(
            "src/server/api/auth.ts(12,5): error TS2322: Type 'string' is not assignable to type 'number'.");
