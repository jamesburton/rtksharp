...
| Fixture file line endings (git normalization) | N/A — this is a fixture-storage concern, not a Rust-vs-RtkSharp behavior difference. | N/A | `tests/parity/fixtures/**/*.txt` and `.json` files were captured on Windows (CRLF) and may be subject to this repo's `.gitattributes` line-ending normalization on commit. **TODO (not yet implemented):** Task 6's `ParityRunner.CompareAsync` (`RtkSharp.ParityTests/ParityRunner.cs`) currently does an exact string comparison (`rustResult.Stdout == dotnetResult.Stdout`) with NO line-ending normalization. This works today only because the test compares the Rust binary against itself (see `ParityTests.cs`'s `CompareAsync_BothToolsReportVersion_ReturnsComparisonResult`, which passes `rustRtkPath` as both the "rust" and "dotnet" paths as a placeholder). A later phase wiring in the actual RtkSharp binary MUST add CRLF/LF normalization before byte-comparing stdout, or cross-platform line-ending differences will produce false-positive mismatches. |
