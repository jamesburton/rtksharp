...
| `Curl` | Passthrough wrapper around real `curl`: full-body passthrough for JSON responses and piped/redirected output, truncate-to-500-bytes-with-tee-hint only for large non-JSON bodies on an interactive terminal | `cmds::cloud::curl_cmd` | **Ported** (see progress ledger) | Cloud/network utility beyond `gh`/`az` reference scope. Corrected description: this row previously said "auto-JSON detection and schema output," but direct source reading (`curl_cmd.rs`) confirms `rtk curl` never parses JSON into a schema or summary — it's a simple size/shape heuristic gating passthrough vs. truncation, not schema inference. |
