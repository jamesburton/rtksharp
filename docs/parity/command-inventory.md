...
| `Next` | Next.js build with compact output | `cmds::js::next_cmd` | Replace with better abstraction | Niche/ecosystem-tool passthrough+filter wrapper not named among CLAUDE.md's JS-stack differentiators (pnpm, vitest, Next.js, TypeScript, Playwright, Prisma — note "Next.js" there refers to framework awareness generally, not this specific `next` build-wrapper command) or as an explicit Phase 4 target; still judged the best fit for eventual declarative-filter migration given its straightforward build-output filtering. |
| `Lint` | ESLint with grouped rule violations | `cmds::js::lint_cmd` | Replace with better abstraction | Niche/ecosystem-tool passthrough+filter wrapper not named among CLAUDE.md's JS-stack differentiators (pnpm, vitest, Next.js, TypeScript, Playwright, Prisma) or as an explicit Phase 4 target; still judged the best fit for eventual declarative-filter migration given its straightforward rule-grouping behavior. |
...
| `Ruff` | Ruff linter/formatter with compact output | `cmds::python::ruff_cmd` | Replace with better abstraction | Niche/rarely-used ecosystem tool passthrough+filter wrapper, not named among CLAUDE.md's stated JS/.NET-first differentiators or as an explicit Phase 4 target; judged the best fit for eventual declarative-filter migration given its straightforward linter-output filtering. |
| `Pytest` | Pytest test runner with compact output | `cmds::python::pytest_cmd` | Replace with better abstraction | Niche/rarely-used ecosystem tool passthrough+filter wrapper, not named among CLAUDE.md's stated JS/.NET-first differentiators or as an explicit Phase 4 target; judged the best fit for eventual declarative-filter migration given its straightforward test-output filtering. |
| `Mypy` | Mypy type checker with grouped error output | `cmds::python::mypy_cmd` | Replace with better abstraction | Niche/rarely-used ecosystem tool wrapper, not named among CLAUDE.md's stated JS/.NET-first differentiators or as an explicit Phase 4 target; judged the best fit for eventual declarative-filter migration given its grouped-error-output pattern is expressible declaratively. |
| `Rake` | Rake/Rails test with compact Minitest output (Ruby) | `cmds::ruby::rake_cmd` | Replace with better abstraction | Niche/rarely-used ecosystem tool passthrough+filter wrapper, not named among CLAUDE.md's stated JS/.NET-first differentiators or as an explicit Phase 4 target; judged the best fit for eventual declarative-filter migration given its straightforward test-output filtering. |
| `Rubocop` | RuboCop linter with compact output (Ruby) | `cmds::ruby::rubocop_cmd` | Replace with better abstraction | Niche/rarely-used ecosystem tool passthrough+filter wrapper, not named among CLAUDE.md's stated JS/.NET-first differentiators or as an explicit Phase 4 target; judged the best fit for eventual declarative-filter migration given its straightforward linter-output filtering. |
| `Rspec` | RSpec test runner with compact output (Rails/Ruby) | `cmds::ruby::rspec_cmd` | Replace with better abstraction | Niche/rarely-used ecosystem tool passthrough+filter wrapper, not named among CLAUDE.md's stated JS/.NET-first differentiators or as an explicit Phase 4 target; judged the best fit for eventual declarative-filter migration given its straightforward test-output filtering. |
| `Pip` | Pip package manager with compact output (auto-detects uv) | `cmds::python::pip_cmd` | Replace with better abstraction | Niche/rarely-used ecosystem tool passthrough+filter wrapper, not named among CLAUDE.md's stated JS/.NET-first differentiators or as an explicit Phase 4 target; judged the best fit for eventual declarative-filter migration given its straightforward install-output filtering. |
...
| `GolangciLint` | golangci-lint wrapper with compact `run` support and passthrough for other invocations | `cmds::go::golangci_cmd` | Replace with better abstraction | Niche/rarely-used ecosystem tool wrapper, not named among CLAUDE.md's stated JS/.NET-first differentiators or as an explicit Phase 4 target; judged the best fit for eventual declarative-filter migration given its straightforward `run`-output filtering plus passthrough for other invocations. |
| `Gradlew` | Android Gradle wrapper with compact output (build, test, lint) | `cmds::jvm::gradlew_cmd` | Replace with better abstraction | Niche/rarely-used ecosystem tool passthrough+filter wrapper, not named among CLAUDE.md's stated JS/.NET-first differentiators or as an explicit Phase 4 target; judged the best fit for eventual declarative-filter migration given its straightforward build-output filtering. |
| `Mvn` | Apache Maven wrapper with compact output (test, integration-test, compile, package, install, verify, deploy) | `cmds::jvm::mvn_cmd` | Replace with better abstraction | Niche/rarely-used ecosystem tool passthrough+filter wrapper, not named among CLAUDE.md's stated JS/.NET-first differentiators or as an explicit Phase 4 target; judged the best fit for eventual declarative-filter migration given its straightforward goal-output filtering across its fixed goal list. |
