...
| Build | `rtk lint` | `biome`, `eslint`, `lint`, + npm/pnpm/npx variants | 84.0 | Replace with better abstraction | Task 1: `Lint` = Replace with better abstraction (not named in JS-stack differentiator list; straightforward rule-grouping fit for declarative migration). |
...
| Build | `rtk next` | `next build` + npm/pnpm/npx variants | 87.0 | Replace with better abstraction | Task 1: `Next` = Replace with better abstraction (CLAUDE.md's "Next.js" differentiator refers to framework awareness generally, not this specific build-wrapper command). |
...
| Build | `rtk mypy` | `python3 -m mypy`, `python -m mypy`, `mypy` | 80.0 | Replace with better abstraction | Task 1: `Mypy` = Replace with better abstraction (grouped-error-output pattern expressible declaratively). |
| Python | `rtk ruff` | `ruff` | 80.0 | Replace with better abstraction | Task 1: `Ruff` = Replace with better abstraction (straightforward linter-output filtering). |
| Python | `rtk pytest` | `python3 -m pytest`, `python -m pytest`, `pytest` | 90.0 | Replace with better abstraction | Task 1: `Pytest` = Replace with better abstraction (straightforward test-output filtering). |
| Python | `rtk pip` | `pip3`, `pip`, `uv pip` | 75.0 | Replace with better abstraction | Task 1: `Pip` = Replace with better abstraction (straightforward install-output filtering). |
| Go | `rtk go` | `go` | 85.0 | Port after core | Task 1: `Go` = Port after core (niche ecosystem beyond fork's stated JS/dotnet/git focus). |
| Go | `rtk golangci-lint run` | `golangci-lint run`, `golangci run` | 85.0 | Replace with better abstraction | Task 1: `GolangciLint` = Replace with better abstraction (straightforward `run`-output filtering plus passthrough). |
...
| Ruby | `rtk rake` | `bundle exec rails`, `bundle exec rake`, `bin/rails`, `rails`, `rake` | 85.0 | Replace with better abstraction | Task 1: `Rake` = Replace with better abstraction (straightforward test-output filtering). |
| Tests | `rtk rspec` | `bundle exec rspec`, `bin/rspec`, `rspec` | 65.0 | Replace with better abstraction | Task 1: `Rspec` = Replace with better abstraction (straightforward test-output filtering). |
| Build | `rtk rubocop` | `bundle exec rubocop`, `rubocop` | 65.0 | Replace with better abstraction | Task 1: `Rubocop` = Replace with better abstraction (straightforward linter-output filtering). |
...
| Build | `rtk gradlew` | `./gradlew`, `gradlew.bat`, `gradlew`, `gradle` | 75.0 | Replace with better abstraction | Task 1: `Gradlew` = Replace with better abstraction (straightforward build-output filtering). |
...
| Build | `rtk mvn` | `./mvnw`, `mvnw.cmd`, `mvnw`, `mvn` | 82.0 | Replace with better abstraction | Task 1: `Mvn` = Replace with better abstraction (straightforward goal-output filtering across fixed goal list). |
